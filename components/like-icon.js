...
export default function LikeButton({ postId, userId }) {
  ...
  function handleLikeUpdate() {
    try {
      updatePostLikeStatus(postId, userId);
      // to re fetch new data
      clearCachesByServerAction(pathName);
    } catch (error) {
      console.error("Error while updating the likes", error);
    }
  }
  return (
    <button className="like-button" onClick={handleLikeUpdate}>
      <svg
